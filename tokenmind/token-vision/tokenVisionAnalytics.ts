...
export class TokenVisionAnalytics {
  compute(series: { timestamp: number; transfers: number; supply: number }[]): Metrics {
    ...
    const std = Math.sqrt(transfers.reduce((s, v) => s + (v - mean) ** 2, 0) / (transfers.length || 1))
    const spikes = series.filter(p => (p.transfers - mean) / (std || 1) > 2).map(p => p.timestamp)
    return { averageTransfers: avgTransfers, maxSupply, transferSpikeTimestamps: spikes }
  }
}
 body: GetBalanceResultBodyType = {
        balance: allBalances,
